...
# ---------------------------------------------------------------------------
# Category grouping - each game carries a "category" and the home screen
# shows one category at a time, switched via the tab bar (see
# draw_category_tabs()) rather than one long scrolling list of sections.
# ---------------------------------------------------------------------------
...
def main():
    ...
    tab_font = pygame.font.Font(str(FONTS_DIR / "Baloo2-Bold.ttf"), 20)
    fonts = {
        "title": title_font,
    ...
    while running:
        ...
        # ---- STATE_MENU: fixed header/tabs + scrolling grid below them ----
        ...
        screen.blit(subtitle_surf, subtitle_surf.get_rect(center=(WIDTH // 2, 104)))

        draw_streak_pill(screen, WIDTH - 24, 22, streak_font, progress["streak_days"])
        gear_hovered = settings_button_rect.collidepoint(mouse_pos)
        draw_settings_button(screen, gear_center, SETTINGS_GEAR_RADIUS, gear_hovered)
        ...
        grid_surface = pygame.Surface((WIDTH, grid_h), pygame.SRCALPHA)
        ...
        for card in cards:
            ...
            draw_card_shadow(grid_surface, rect, CARD_RADIUS, hovered)
