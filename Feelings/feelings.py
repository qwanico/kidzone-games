...
def load_feelings():
    return sorted(p.stem for p in VOICE_DIR.glob("*.mp3"))
...
def load_animations(feelings, size):
    return {name: GifAnimation(ANIM_DIR / f"{name}.gif", size) for name in feelings}
...
class Game:
    def __init__(self):
        ...
        self.animations = load_animations(self.feelings, ANIM_SIZE)
    ...
    def speak(self, feeling):
        ...
        pygame.mixer.music.load(str(VOICE_DIR / f"{feeling}.mp3"))
