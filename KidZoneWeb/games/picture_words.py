...
class Game:
    def __init__(self):
        ...
        if platform is not None and hasattr(platform, "window"):
            try:
                ...
                pass
        pygame.display.set_caption("Which Word")
        ...
        if len(self.names) < 2:
            raise RuntimeError("Need at least 2 picture/word pairs in assets/")
        ...
        self.card_rect = pygame.Rect(0, 0, PICTURE_SIZE + 60, PICTURE_SIZE + 60)
        ...
        # --- decorative background (warm drifting bokeh glows) ---
        self.bg_surface = build_gradient(WIDTH, HEIGHT, BG_TOP_COLOR, BG_BOTTOM_COLOR)
        self.bokeh = [self._make_bokeh(initial=True) for _ in range(12)]
    ...
    def _make_bokeh(self, initial=False):
        return {
    ...
    def update_background(self, dt_ms):
        ...
        for b in self.bokeh:
            ...
            if b["y"] < -50:
                b["y"] = HEIGHT + random.uniform(0, 50)
    ...
    def load_picture(self, name):
        img = pygame.image.load(str(self.items[name]["image"])).convert_alpha()
        w, h = img.get_size()
    ...
    def speak(self, name):
        ...
        pygame.mixer.music.load(str(self.items[name]["voice"]))
        pygame.mixer.music.set_volume(1.0)
        pygame.mixer.music.play()

    def new_round(self):
        choices = [n for n in self.names if n != self.last_name] or self.names
        self.current_name = random.choice(choices)
        self.last_name = self.current_name

        wrong_choices = [
            n for n in self.names
            if n != self.current_name and n != self.last_wrong
        ] or [n for n in self.names if n != self.current_name]
        wrong_name = random.choice(wrong_choices)
        self.last_wrong = wrong_name

        self.picture_surface = self.load_picture(self.current_name)

        pair = [self.current_name, wrong_name]
        random.shuffle(pair)
    ...
    def handle_click(self, pos):
        ...
        for button, name in zip(self.buttons, self._button_names):
            if button.rect.collidepoint(pos):
                is_correct = name == self.current_name
                for b, n in zip(self.buttons, self._button_names):
                    b.state = "correct" if n == self.current_name else (
                ...
                if is_correct:
                    ...
                    self.feedback_text = f"That's {to_display_name(self.current_name)}!"
    ...
    def draw_menu(self):
        ...
        title_surf = self.font_title.render("Which Word", True, TITLE_COLOR)
        title_rect = title_surf.get_rect(center=(WIDTH // 2, 220))
        self.screen.blit(title_surf, title_rect)

        subtitle_surf = self.font_subtitle.render(
            "Pick the word that matches the picture!", True, TEXT_COLOR
        )
    ...
    def draw_playing(self):
        ...
        pic_rect = self.picture_surface.get_rect(center=card_rect.center)
        self.screen.blit(self.picture_surface, pic_rect)
