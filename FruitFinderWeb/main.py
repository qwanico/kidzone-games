...
pygame.display.set_icon(
    pygame.image.load("assets/app_icon.png")
)
...
# ---------------- VOICE ----------------
# All voice lines are pre-generated and shipped as .ogg in voice_cache/ -
# no on-the-fly TTS generation here, since there's no gTTS/network call
# available inside the browser's WASM Python runtime.
...
def speak_fruit(fruit):
    ...
    if not os.path.exists(path):
        print(f"No cached voice line for {fruit}, skipping")
        return 0
...
def speak_callout():
    ...
    if not os.path.exists(path):
        print(f"No cached callout line for {key}, skipping")
        return
...
correct_sounds = [
    pygame.mixer.Sound("sounds/correct.ogg"),
    pygame.mixer.Sound("sounds/chime1.ogg"),
    pygame.mixer.Sound("sounds/chime2.ogg"),
]
...
background_music = pygame.mixer.Sound("sounds/background_music.ogg")
